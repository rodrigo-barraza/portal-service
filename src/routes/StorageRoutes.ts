...
function objectTarget(req: Request): {
  bucketName: string;
  objectName: string;
} {
...
router.get("/buckets/stream", async (_req: Request, res: Response) => {
  ...
  try {
    ...
    logger.error(
      `[ObjectStore] streamBuckets failed: ${getErrorMessage(error)}`,
    );
    if (!clientGone) {
      res.write(
...
router.get("/buckets/:name", async (req: Request, res: Response) => {
  ...
  try {
    const result = await MinioService.listObjects(
      bucketName,
      prefix,
      recursive,
    );
...
router.get(
  "/buckets/:name/download/*objectPath",
  async (req: Request, res: Response) => {
    const { bucketName, objectName } = objectTarget(req);
    ...
    const contentType = String(
      stat.metaData?.["content-type"] || guessMime(objectName),
    );
    ...
    const stream =
      range.kind === "range"
        ? await MinioService.getPartialObject(
            bucketName,
            objectName,
            range.start,
    ...
    stream.on("error", (streamError: unknown) => {
      logger.error(
        `[ObjectStore] stream failed mid-transfer: ${getErrorMessage(streamError)}`,
      );
      res.destroy();
    });
