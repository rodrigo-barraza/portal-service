...
beforeEach(() => {
  ...
  minio.getObject.mockImplementation(async () => Readable.from([Buffer.from("hello world")]));
  minio.getPartialObject.mockImplementation(async () => Readable.from([Buffer.from("world")]));
});

describe("nested object keys", () => {
  it("stat receives the key with its slashes intact", async () => {
    const response = await fetch(`${baseUrl}/buckets/images/stat/folder/sub/file%20one.png`);
    expect(response.status).toBe(200);
    expect(minio.statObject).toHaveBeenCalledWith("images", "folder/sub/file one.png");
  ...
  it("delete targets the real key, not a comma-joined one", async () => {
    ...
    const response = await fetch(`${baseUrl}/buckets/images/2026/09/a.png`, { method: "DELETE" });
    expect(response.status).toBe(200);
  ...
  it("rejects traversal segments before MinIO sees them", async () => {
    expect(await rawGetStatus("/object-store/buckets/images/stat/a/%2E%2E/b")).toBe(400);
    expect(await rawGetStatus("/object-store/buckets/images/stat/a/../../other/key")).toBe(400);
    expect(minio.statObject).not.toHaveBeenCalled();
  });

  it("maps a missing key to 404 with a string error", async () => {
    minio.statObject.mockRejectedValue(Object.assign(new Error("Not Found"), { code: "NotFound" }));
...
describe("download", () => {
  it("streams the object with safe headers", async () => {
    const response = await fetch(`${baseUrl}/buckets/images/download/pages/index.html?inline=true`);
    expect(response.status).toBe(200);
    expect(await response.text()).toBe("hello world");
    expect(response.headers.get("content-disposition")).toContain(`inline; filename="index.html"`);
  ...
  it("serves a single byte range", async () => {
    ...
    expect(minio.getPartialObject).toHaveBeenCalledWith("images", "a/b.txt", 6, 5);
  });
...
describe("validation", () => {
  ...
  it("clamps a junk search limit instead of scanning unbounded", async () => {
    minio.searchObjects.mockResolvedValue({ results: [], totalScanned: 0, truncated: false });
    await fetch(`${baseUrl}/search?query=cat&limit=abc`);
    expect(minio.searchObjects).toHaveBeenCalledWith("cat", { bucket: undefined, limit: 200 });
  });

  it("hides an unexpected MinIO failure behind a generic 500", async () => {
    minio.listObjects.mockRejectedValue(new Error("connect ECONNREFUSED 10.1.2.3:9000"));
