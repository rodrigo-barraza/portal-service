...
describe("assertValidBucketName", () => {
  ...
  it("rejects malformed names with a 400", () => {
    for (const name of ["", "ab", "UPPER", "-lead", "trail-", "a..b", "a/b", "x".repeat(64)]) {
...
describe("assertValidObjectName", () => {
  it("accepts nested keys", () => {
    expect(() => assertValidObjectName("2026/09/photo.final.png")).not.toThrow();
    expect(() => assertValidObjectName("a,b (1).txt")).not.toThrow();
  });

  it("rejects empty, traversal, and NUL keys", () => {
    for (const name of ["", "..", "../other-bucket/key", "a/./b", "a/../b", "a\0b", "x".repeat(1025)]) {
...
describe("contentDisposition", () => {
  ...
  it("percent-encodes RFC 5987 delimiters and non-ASCII in filename*", () => {
    ...
    expect(header).toContain(`filename*=UTF-8''it%27s%20%28caf%C3%A9%29%2A.txt`);
    expect(header).toContain(`filename="it's (caf_)*.txt"`);
  });

  it("falls back to 'download' without a name", () => {
    expect(contentDisposition("attachment", undefined)).toContain(`filename="download"`);
  });
});
...
describe("parseRangeHeader", () => {
  ...
  it("flags unsatisfiable ranges", () => {
    ...
    expect(parseRangeHeader("bytes=20-10", 100)).toEqual({ kind: "unsatisfiable" });
...
describe("toObjectStoreError", () => {
  it("maps missing buckets/keys to 404 and bad names to 400", () => {
    const missingKey = toObjectStoreError(Object.assign(new Error("Not Found"), { code: "NotFound" }));
    expect(missingKey).toBeInstanceOf(HttpError);
    expect((missingKey as HttpError).statusCode).toBe(404);

    const missingBucket = toObjectStoreError(Object.assign(new Error("x"), { code: "NoSuchBucket" }));
    expect((missingBucket as HttpError).message).toBe("Bucket not found");

    const badName = toObjectStoreError(Object.assign(new Error("Invalid bucket name: X"), { name: "InvalidBucketNameError" }));
