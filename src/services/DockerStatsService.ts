...
import ContainerMetricsService, { PERSIST_INTERVAL_MS } from "./ContainerMetricsService.ts";
import type { DeviceEntry, DeviceSpecs, ContainerStats, ContainerSnapshot } from "../types.ts";
...
export default class DockerStatsService {
  public static async getAll(deviceId?: string): Promise<ContainerStats[]> {
    ...
    const targets = deviceId ? devices.filter((deviceEntry) => deviceEntry.id === deviceId) : devices;
  ...
  private static async _getAllForDevice(deviceId: string, deviceEntry: DeviceEntry): Promise<ContainerStats[]> {
    try {
  ...
  private static async _fetchAllForDevice(deviceId: string, deviceEntry: DeviceEntry): Promise<ContainerStats[]> {
    try {
      const containers = await DockerClient.dockerGetJson<Record<string, unknown>[]>(
        deviceEntry,
      ...
      const stats = await Promise.all(
        containers.map(async (containerEntry): Promise<ContainerStats | null> => {
          ...
          try {
            const rawStats = await DockerClient.dockerGetJson<Record<string, unknown>>(
              deviceEntry,
      ...
      const activeIds = new Set(containers.map((containerEntry) => String(containerEntry.Id)));
      for (const id of deviceCounters.keys()) {
        if (!activeIds.has(id)) deviceCounters.delete(id);
      }

      return stats.filter((entry): entry is ContainerStats => entry !== null).sort(byName);
    } catch (error: unknown) {
  ...
  public static startCollector(): void {
    ...
    collectorTimer = setInterval(() => void DockerStatsService._collectSnapshot(), HISTORY_INTERVAL_MS);
  ...
  public static async _collectSnapshot(): Promise<void> {
    await Promise.all(
      getDockerDevices().map(({ id, device }) => DockerStatsService._collectDeviceSnapshot(id, device)),
    );
  }
  ...
  public static async _collectDeviceSnapshot(deviceId: string, device: DeviceEntry): Promise<void> {
    ...
    try {
      const stats = await DockerStatsService._fetchAllForDevice(deviceId, device);
      statsCache.set(deviceId, stats);

      const snapshot: ContainerSnapshot = { timestamp: new Date().toISOString(), containers: {} };
      ...
      if (history.length > HISTORY_MAX_SAMPLES) history.splice(0, history.length - HISTORY_MAX_SAMPLES);
      historyMap.set(deviceId, history);

      const now = Date.now();
      if (now - (lastPersistMap.get(deviceId) || 0) >= PERSIST_INTERVAL_MS) {
        lastPersistMap.set(deviceId, now);
        ContainerMetricsService.persistSnapshot(deviceId, stats).catch((error: unknown) => {
          logger.warn(`[DockerStats:${deviceId}] Metrics persist failed: ${getErrorMessage(error)}`);
        });
      }
    } catch (error: unknown) {
  ...
  public static getSystemInfo(deviceId?: string): Promise<unknown> {
    return systemInfoCache.get(deviceId || "__all__", SYSTEM_INFO_CACHE_TTL_MS, () =>
  ...
  private static async _fetchSystemInfo(deviceId?: string) {
    ...
    if (deviceId) {
      ...
      if (!target) throw new HttpError(`Unknown Docker device: ${deviceId}`, 404);
      return DockerSystemHelper.getSystemInfoForDevice(target.id, target.device, SYSTEM_REQUEST_TIMEOUT_MS);
    }
    ...
    return results
      .filter((result): result is PromiseFulfilledResult<Record<string, unknown> & { deviceName: string }> =>
