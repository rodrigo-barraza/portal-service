...
describe("GoogleCloudUsageService metadata", () => {
  it("uses the full Cloud Monitoring labels for Maps Platform backends", () => {
    ...
    expect(
      KNOWN_API_METADATA["geocoding-backend.googleapis.com"],
    ).toBeDefined();
    expect(
      KNOWN_API_METADATA["static-maps-backend.googleapis.com"],
    ).toBeDefined();
  ...
  it("tracks the Gemini API", () => {
    expect(
      KNOWN_API_METADATA["generativelanguage.googleapis.com"]?.displayName,
    ).toBe("Gemini API");
  });
  ...
  it("derives a readable fallback for unknown services instead of hiding them", () => {
    ...
    expect(metadata.documentationUrl).toContain(
  ...
  it("never excludes a service that also has curated metadata", () => {
    for (const serviceIdentifier of Object.keys(KNOWN_API_METADATA)) {
      expect(EXCLUDED_INFRASTRUCTURE_SERVICES.has(serviceIdentifier)).toBe(
        false,
      );
    }
  });
...
describe("GoogleCloudUsageService.isValidServiceIdentifier", () => {
  it("accepts real service identifiers", () => {
    expect(
      GoogleCloudUsageService.isValidServiceIdentifier("places.googleapis.com"),
    ).toBe(true);
    expect(
      GoogleCloudUsageService.isValidServiceIdentifier(
  ...
  it("rejects monitoring-filter injection attempts", () => {
    ...
    expect(
      GoogleCloudUsageService.isValidServiceIdentifier('x" OR metric.type = "'),
    ).toBe(false);
    expect(
      GoogleCloudUsageService.isValidServiceIdentifier("UPPER.googleapis.com"),
    ).toBe(false);
  });
});
...
describe("prettifyServiceIdentifier", () => {
  it("strips the googleapis suffix and title-cases the rest", () => {
    expect(prettifyServiceIdentifier("generativelanguage.googleapis.com")).toBe(
      "Generativelanguage",
    );
    expect(
      prettifyServiceIdentifier("static-maps-backend.googleapis.com"),
    ).toBe("Static Maps Backend");
  });
});
