...
import ContainerMetricsService, { metricsSeriesKey } from "../ContainerMetricsService.ts";

const point = (cpu: number, t: string) => ({ t: new Date(t), cpu, mem: 1, memLimit: 2, netRx: 0, netTx: 0, pids: 1 });
...
describe("ContainerMetricsService.getHistory", () => {
  it("keeps same-named containers on different devices apart", async () => {
    aggregate.mockReturnValue({
      toArray: async () => [
        { _id: { container: "watchtower", device: "synology" }, points: [point(2, "2026-09-22T10:01:00Z"), point(1, "2026-09-22T10:00:00Z")] },
        { _id: { container: "watchtower", device: "workstation2" }, points: [point(9, "2026-09-22T10:00:00Z")] },
      ],
    });

    const history = await ContainerMetricsService.getHistory({ range: "1h" });

    expect(Object.keys(history.containers).sort()).toEqual(["synology/watchtower", "workstation2/watchtower"]);
    expect(history.containers[metricsSeriesKey("workstation2", "watchtower")]).toMatchObject({
      container: "watchtower",
      device: "workstation2",
    });
    // $topN hands back newest-first; the series is chronological
    expect(history.containers["synology/watchtower"].points.map((entry) => entry.cpu)).toEqual([1, 2]);
  ...
  it("caps the lookback at the collection's 7-day TTL", async () => {
    ...
    const history = await ContainerMetricsService.getHistory({ range: "99999999d" });
  ...
  it("filters by container and device when asked", async () => {
    aggregate.mockReturnValue({ toArray: async () => [] });
    await ContainerMetricsService.getHistory({ container: "prism-service", device: "synology" });
