...
describe("extractRepoSlug", () => {
  it("reads owner/repo from GitHub URLs", () => {
    expect(
      extractRepoSlug("https://github.com/rodrigo-barraza/portal-service"),
    ).toBe("rodrigo-barraza/portal-service");
    expect(
      extractRepoSlug("https://github.com/rodrigo-barraza/portal-service.git"),
    ).toBe("rodrigo-barraza/portal-service");
    expect(
      extractRepoSlug("https://github.com/rodrigo-barraza/portal-service/"),
    ).toBe("rodrigo-barraza/portal-service");
...
describe("toLanguageBreakdown", () => {
  it("sorts by bytes and rounds percentages to one decimal", () => {
    const breakdown = toLanguageBreakdown({
      CSS: 100,
      TypeScript: 800,
      JavaScript: 100,
    });
    ...
    expect(breakdown.breakdown.map((entry) => entry.language)).toEqual([
      "TypeScript",
      "CSS",
      "JavaScript",
    ]);
