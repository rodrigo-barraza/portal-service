...
export function parseDfOutput(
  dfOutput: string,
): { total: number; used: number; available: number; percent: number } | null {
...
export class DockerSystemHelper {
  ...
  public static async getSpecsForDevice(
    ...
    const infoResponseBody = await DockerClient.dockerGet(
      deviceEntry,
      "/info",
      timeoutMs,
    );
  ...
  public static async getSystemInfoForDevice(
    ...
    const imagesMapped = (
      (diskFreeStats.Images as Record<string, unknown>[]) || []
    ).map((imageEntry: Record<string, unknown>) => ({
    ...
    const totalImageSize = imagesMapped.reduce(
      (sum, image) => sum + image.size,
      0,
    );
    const totalImageShared = imagesMapped.reduce(
      (sum, image) => sum + image.sharedSize,
      0,
    );

    const volumesMapped = (
      (diskFreeStats.Volumes as Record<string, unknown>[]) || []
    ).map((volumeEntry: Record<string, unknown>) => ({
      ...
      refCount:
        (volumeEntry.UsageData as Record<string, number>)?.RefCount || 0,
    }));

    const totalVolumeSize = volumesMapped.reduce(
      (sum, volume) => sum + volume.size,
      0,
    );

    const buildCacheEntries =
      (diskFreeStats.BuildCache as Record<string, unknown>[]) || [];
    const totalBuildCacheSize = buildCacheEntries.reduce(
      (sum, entry) => sum + ((entry.Size as number) || 0),
      0,
    );

    const containersDiskFree = (
      (diskFreeStats.Containers as Record<string, unknown>[]) || []
    ).map((containerEntry: Record<string, unknown>) => ({
    ...
    return {
      ...
      totalMemory:
        infoParsed.MemTotal ||
        (deviceEntry.dockerApi?.startsWith("unix://") ? os.totalmem() : 0),
      cpus:
        infoParsed.NCPU ||
        (deviceEntry.dockerApi?.startsWith("unix://") ? os.cpus().length : 0),
      ...
      disk: {
        images: {
          ...
          items: imagesMapped
            .sort(
              (firstImage, secondImage) => secondImage.size - firstImage.size,
            )
            .slice(0, 20),
        },
        volumes: {
          ...
          items: volumesMapped.sort(
            (firstVolume, secondVolume) => secondVolume.size - firstVolume.size,
          ),
        },
        buildCache: {
