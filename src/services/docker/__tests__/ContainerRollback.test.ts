...
async function fakeRequest(
  ...
  const failure =
    engine.failures[route] ?? engine.failures[`${method} ${url.pathname}`];
  if (failure === "throw") throw new Error(`Docker API timeout (${route})`);
  if (failure === "status")
    return reply(500, { message: `injected failure: ${route}` });
  ...
  if (segments[0] === "images" && segments[2] === "json") {
    ...
    return imageId
      ? reply(200, {
          ...
          Size: 10,
          Config: { Labels: { "git.sha": "abc" } },
        })
      : reply(404);
  }
  if (segments[0] === "images" && segments[2] === "tag") {
    const imageId = engine.images.has(segments[1])
      ? segments[1]
      : engine.tags.get(segments[1]);
    if (!imageId) return reply(404);
    engine.tags.set(
      `${url.searchParams.get("repo")}:${url.searchParams.get("tag")}`,
      imageId,
    );
    return reply(201);
  }
...
describe("rollbackToPreviousImage", () => {
  it("swaps the tags and replaces the container", async () => {
    ...
    expect(engine.containers[0]).toMatchObject({
      name: "notes-service",
      running: true,
    });
  ...
  it("refuses with a 400 when there is no :previous image", async () => {
    ...
    await expect(
      rollbackToPreviousImage(device, "notes-service", "notes-service"),
  ...
  it("removes a replacement that fails to start and brings the original back", async () => {
    ...
    vi.mocked(DockerClient.dockerRequest).mockImplementation(
      async (deviceEntry, method, path, options) => {
        if (
          method === "POST" &&
          path.startsWith("/containers/c-original/start")
        ) {
